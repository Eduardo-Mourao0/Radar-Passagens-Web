...
export function createRouteCard(route) {
  ...
  const originCity = escapeHtml(getAirportCity(route.origem));
  const destinationCity = escapeHtml(getAirportCity(route.destino));

  return `<article class="route-card ${route.ativa ? '' : 'inactive'}"><div class="card-top"><div class="route-code"><div class="route-airport"><span class="route-city">${originCity}</span><span class="route-iata">${origin}</span></div><span class="route-arrow">\u2192</span><div class="route-airport"><span class="route-city">${destinationCity}</span><span class="route-iata">${destination}</span></div></div><span class="badge">${route.ativa ? 'ATIVA' : 'PAUSADA'}</span></div><p class="dates">${escapeHtml(dates)}</p><div class="price-line"><div><div class="price-label">\u00daLTIMA TARIFA</div><div class="price ${price ? '' : 'empty'}">${escapeHtml(fare)}</div></div>${price ? `<span class="price-label">${escapeHtml(price.companhia)}</span>` : ''}</div><p class="alert-status ${route.alertaPreco ? 'configured' : ''}">${alertLabel}</p><div class="card-actions"><button data-action="history" data-id="${escapeHtml(route.id)}">Hist\u00f3rico</button><button data-action="alert" data-id="${escapeHtml(route.id)}" data-route="${origin} \u2192 ${destination}">Alerta</button><button data-action="booking" data-id="${escapeHtml(route.id)}">Ver passagem</button><button data-action="toggle" data-id="${escapeHtml(route.id)}" data-active="${route.ativa}">${route.ativa ? 'Pausar' : 'Reativar'}</button><button class="delete-button" data-action="delete" data-id="${escapeHtml(route.id)}">Excluir</button></div></article>`;
}
